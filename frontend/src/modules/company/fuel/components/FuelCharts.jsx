...
function FuelCharts({ pie }) {
  return (
    <div className="min-w-0">
      <h3 className="text-sm font-semibold text-zinc-100">Consumo por veículo</h3>
      <p className="mt-1 text-xs leading-relaxed text-zinc-500">Proporção de litros por veículo (diagrama radial).</p>
      <div className="mt-5 flex flex-col items-stretch gap-6 sm:flex-row sm:items-center">
        <div
          className="mx-auto h-52 w-52 shrink-0 rounded-full border-4 border-zinc-800 shadow-lg ring-1 ring-zinc-700/40"
          style={pie.style}
          role="img"
        ...
        <ul className="min-w-0 flex-1 space-y-2.5">
          {pie.slices.length === 0 ? (
            ...
            pie.slices.map((s, idx) => (
              <li key={`pie-slice-${idx}`} className="flex items-center gap-2 text-sm text-zinc-300">
                <span
