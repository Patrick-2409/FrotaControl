...
export default function AdminLoginPage() {
  const { adminEmpresaLogin, logout } = useAuth();
  ...
  const onSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const user = await adminEmpresaLogin({ login: normalizedLogin, senha: normalizedSenha });
      ...
      if (status === 401) {
        ...
        setError(err.response?.data?.message || "Não foi possível fazer login. Verifique os dados e tente novamente.");
      }
    } finally {
  ...
  return (
    <div className="fc-page grid min-h-screen place-content-center bg-slate-950 p-6">
      {submitting && <CenteredSpinner label="Entrando no dashboard..." />}
      <form onSubmit={onSubmit} className="fc-card w-full max-w-sm p-6">
        <div className="mb-4 flex justify-center">
          <SystemLogo variant="auth" />
        </div>
        <h1 className="mb-1 text-2xl font-bold text-white">Acesso Empresa</h1>
        <p className="mb-5 text-sm text-slate-400">Painel de gestão de motoristas, veículos e relatórios</p>
        ...
        <FormField label="Login (CPF, e-mail ou ID)">
          <input
            ...
            placeholder="Ex.: 123.456.789-00, admin@empresa.com ou USR-000123"
        ...
        <button className={primaryButtonClass} disabled={submitting}>Entrar no dashboard</button>
